...
3. Home directory config file (`~/.ziit.json`)
4. Default values
...
Extensions should migrate legacy config files (`.ziit.cfg`) to the new JSON format automatically.
...
For detailed API endpoint information, see [API Integration](#api-integration).
...
### Offline Queue
- Store failed heartbeats in `~/.ziit/offline_heartbeats.json`
- Automatically sync when connection is restored
- Batch sync in groups of 1000 heartbeats (see [API Integration](#api-integration) for batch endpoint)
...
**Batch Sync (for offline heartbeats):**
```http
