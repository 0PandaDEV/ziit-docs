...
description: A spec sheet on how to building IDE extensions for Ziit.
icon: Layers
---
...
3. Home directory config file (`~/.config/ziit/config.json`)
4. Default values

**Migration: Migrate from the old format of configuration file (`~/.ziit.json`) to the new format (`~/.config/ziit/config.json`) on first run.**
...
- Store failed heartbeats in `~/.config/ziit/offline_heartbeats.json`
...
**Migration: Migrate from the old format of configuration file (`~/.ziit/offline_heartbeats.json`) to the new format (`~/.config/ziit/offline_heartbeats.json`) on first run.**
...
- **Normal**: `⏰ X hrs Y mins` - Shows today's tracked time
- **Offline**: `🔄 X hrs Y mins (offline)` - Indicates offline mode
- **Unconfigured**: `❌ Unconfigured` - Missing/invalid API key
